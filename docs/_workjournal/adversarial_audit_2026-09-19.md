...
| E10 | HIGH | **OPEN — needs Curro** | **GitHub Actions is disabled repository-wide.** `repos/fmarrabal/openmiura/actions/permissions` returns `{"enabled": false}`; every workflow still reports `state: active`, so nothing looks wrong from the workflows page, but no run has executed since **2026-08-09** (PRs #164/#165 were the last with real CI). Only GitGuardian still reports on a PR, because it is an external app rather than an Actions workflow. Consequence: the quality gate, the 3.10/3.11/3.12 matrix, `pip-audit`, the secret scan and the reproducible-package check have not run for over a month, and a PR can show a green tick from GitGuardian alone. For a project whose entire claim is verifiable governance, having its own automated verification silently switched off is the sharpest possible instance of the gap this audit is about. Not changed here: re-enabling Actions is a repository setting that may be off deliberately (billing, minutes, or a deliberate response), so it is the owner's call. |
