# Documentation index

## Public starting points

- [Public narrative](public_narrative.md)
- [Installation guide](installation.md)
- [Canonical demo](demos/canonical_demo.md)
- [Canonical runtime governance walkthrough](walkthroughs/canonical_runtime_governance_walkthrough.md)
- [Stable release publication policy](release_publication.md)

## Public media pack

- [Screenshot plan](media/screenshot_plan.md)
- [Medium article outline](media/medium_article_outline.md)
- [Medium article draft](media/medium_article_draft.md)
- [Medium article final](media/medium_article_final.md)
- [Medium article publication pack](media/medium_article_publication_pack.md)
- [Stable release text pack](media/stable_release_text_pack.md)
- [Stable release final pack](release/stable_release_final_pack.md)
- [Publication reuse note](media/publication_reuse_note.md)

## Product and platform guides

- [Extension SDK](extensions_sdk.md)
- [Private extension registry](extensions_registry.md)
- [Production guide](production.md)
- [Backup and restore](backup_restore.md)
- [Migrations](migrations.md)
- [Security guide](security.md)
- [LLM providers](llm_providers.md)
- [MCP and HTTP broker integration](mcp_broker_integration.md)
- [Science profile — NMR parsers and vendor support](science_nmr_parsers.md)
- [Troubleshooting](troubleshooting.md)

## Operational guides

- [Deployment](deployment.md)
- [Observability](observability.md)
- [MCP](mcp.md)
- [CI/CD](ci_cd.md)
- [Channel configuration examples](channel-config-examples.md)
- [Alert runbook](runbooks/alerts.md)

## Release and adoption docs

- [Self-hosted Enterprise Alpha](enterprise_alpha.md)
- [Alpha release checklist](alpha_release_checklist.md)
- [Release Candidate RC1](release_candidate.md)
- [Release support matrix](release_support_matrix.md)
- [RC1 closure note](release_candidate_closure.md)
- [Final PR / merge / publish checklist](github_pr_merge_publish_checklist.md)

## Historical and roadmap material

- [24-month product strategy roadmap](ROADMAP_24M_PRODUCT_STRATEGY.md)
- [Refactor closure and artifacts](refactor/README.md)
